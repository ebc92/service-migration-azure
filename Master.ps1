...
#Runs the required script to start the migration of the service and deployment of the new VM
Invoke-Command -Session $AzureStackSession -ScriptBlock {New-AzureStackTenantDeployment -VMName $using:ExchName -IPAddress $using:ExchNewIp `
-DomainCredential $using:DomainCredential -VMSize "Standard_D4_v2"}
